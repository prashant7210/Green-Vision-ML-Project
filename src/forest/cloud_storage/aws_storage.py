import boto3
from src.forest.configurations.aws_connection import S3Client
...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key) -> bool:
        try:
            ...
            if len(file_objects) > 0: return True
            else: return False
    ...
    def read_object(object_name: object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        ...
        try:
            ...
            if object_name is None:
                raise ForestExpection("Object is None. Cannot read from None object.", sys)
            ...
            if isinstance(object_name, list):
                ...
                if len(object_name) > 0:
                    ...
                    raise ForestExpection("Empty list of objects provided", sys)

            # Check if object has get method
            if not hasattr(object_name, 'get'):
                raise ForestExpection(f"Object of type {type(object_name)} does not have 'get' method", sys)

            # Get the object content
            response = object_name.get()
            if not response or 'Body' not in response:
                raise ForestExpection("Invalid response from S3 object get() method", sys)
    ...
    def get_file_object( self, filename: str, bucket_name: str) -> Union[List[object], object]:
        ...
        try:
            bucket = self.get_bucket(bucket_name)
            if bucket is None:
                raise ForestExpection(f"Bucket {bucket_name} not found", sys)
            ...
            if len(file_objects) == 0:
                ...
                raise ForestExpection(f"No objects found with prefix {filename} in bucket {bucket_name}", sys)
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None) -> object:
        ...
        try:
            ...
            logging.info(f"Exited the load_model method of S3Operations class")
            return model
    ...
    def upload_file(self, from_filename: str, to_filename: str,  bucket_name: str,  remove: bool = True):
        ...
        try:
            ...
            else: logging.info(f"Remove is set to {remove}, not deleted the file")
